...
Reset_Handler   PROC
                ...
                IMPORT  SystemInit
                IMPORT  __main
				IMPORT  _systemcall_table_init ; import _systemcall_table_init from SVC.s
				IMPORT  _heap_init ; import _heap_init from heap.s
				; set up MSP, do not change the following three lines
				LDR     R0, =__initial_sp ; thread mode uses MSP
				MSR     MSP, R0 ; model-specific register is the Master Stack Pointer/ SVC stack (MSP) with the address of R0
				; Move R0 to the Master Stack Pointer and make it to be model-specific register
				ISB  ;Instruction Synchronization Barrier, flushes the pipeline of the processor.
				
                LDR     R0, =SystemInit
                BLX     R0
				LDR     R0, =_heap_init ; Load Ro to have the address of _heap_init
				BLX     R0				; Branch the Register R0 which is _heap_init, it calls the function _heap_init from heap.s
				LDR     R0, =_systemcall_table_init ; Load Ro to have the address of _systemcall_table_init
				BLX     R0				; Branch the Register R0 which is _systemcall_table_init, it calls _systemcall_table_init from svc.s
				
				; set up thread stack, do not change any code from here to ENDP
				LDR     R0, =__initial_user_sp ; Load R0 to have the address of __initial_user_sp
				MSR     PSP, R0 		; model-specific register is the Process Stack Pointer/ User Stack (PSP) with the address of R0
				;Move R0 to the Process Stack Pointer, and make it to be model-specific register
...
SVC_Handler     PROC
                ...
                MRS	R1, PSP						; system coprocessor register to ARM register R1 with the Process stack pointer
				STR	R0, [R1]					; Store the address of R0 to memory of R1 which is the Process stack pointer
